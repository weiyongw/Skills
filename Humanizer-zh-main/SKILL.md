...
name: humanizer-zh
description: |
  去除中文文本中的 AI 生成痕迹，使其更自然、更有人味。基于维基百科 AI 写作特征指南，覆盖 23 类常见 AI 写作模式。
allowed-tools:
  - Read
  - Write
  - Edit
  - AskUserQuestion
metadata:
  trigger: 去除AI痕迹、人性化文本、编辑AI生成的文本、让文字更自然、去除机器味、humanize
...
**问题：** ChatGPT 使用弯引号（\u201c\u201d）而不是直引号（""）。中文引号（「」或""）不受此规则影响，仅在文本含英文引号时检查。
...
以表格形式呈现所有修改，让用户直观了解改进内容：

| 原文片段 | 问题类型 | 改写结果 |
|----------|----------|----------|
| 作为……的证明 | #1 夸大象征意义 | 直接陈述功能 |
| 此外 | #7 AI 词汇 | 删除 |
| 无缝、直观和强大 | #10 三段式法则 | 列出具体功能 |

表格要求：
- **原文片段**：摘出被修改的原始文字（简短引用即可）
- **问题类型**：标注对应的模式编号和名称（如 #7 AI 词汇）
- **改写结果**：简述如何处理（改写为什么 / 删除 / 合并等）
...
| **真实性** | 听起来像真人说话吗？ | /10 |
| **精炼度** | 还有可删减的冗余吗？ | /10 |
| **总分** | 45-50 优秀 / 35-44 良好 / <35 需重改 | **/50** |
...
| 原文片段 | 问题类型 | 改写结果 |
|----------|----------|----------|
| 作为公司致力于创新的证明 | #1 夸大象征意义 | 删除，直接说功能 |
| 此外 | #7 AI 词汇 | 删除 |
| 无缝、直观和强大 | #10 三段式法则 + #4 宣传语言 | 替换为具体功能名 |
| ——确保用户能够高效地完成目标 | #3 -ing 肤浅分析 | 删除破折号和补充短语 |
| 这不仅仅是……而是…… | #9 否定式排比 | 删除 |
| 行业专家认为 | #5 模糊归因 | 替换为"测试用户反馈" |
| 关键作用 / 不断演变的格局 | #7 AI 词汇 | 删除 |
